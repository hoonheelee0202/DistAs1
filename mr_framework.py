...
def barrier_sink (args):
    ...
    try:
        ...
        while (i < args['cond']):
		    ...
		    ### Modify map_file.write function to reflect the change of the data format -- hhlee
                    map_file.write(str(map_resp[j]['house_id']) + str (",") + str(map_resp[j]['household_id']) + str(",") + str(map_resp[j]['plug_id']) + str(",") + str(map_resp[j]['property']) + str(",") + str(map_resp[j]['value']) + str(",") + str(map_resp[j]['timestamp']) + str(",") + str(map_resp[j]['id']) + "\n")
		...
		print("reduce length: "+str(len(reduce_resp)))

                # write the csv entries to the file.
                for j in range (len(reduce_resp)):
		    reduce_file.write (str(reduce_resp[j]['house_id']) + str (",") + str(reduce_resp[j]['household_id']) + str(",") + str(reduce_resp[j]['plug_id']) + str(",") + str(reduce_resp[j]['property']) + str(",") + str(reduce_resp[j]['sum']) + str(",") + str(reduce_resp[j]['time']) + str(",") + str(reduce_resp[j]['group_count']) + str(",") + str(reduce_resp[j]['avg_value']) + "\n")
...
class MR_Framework ():
    ...
    def shuffle_func (self):
	    ...
	    ### itemgetter(0) -> itemgetter(0,1,2,3) to sort by house_id, household_id, plug_id, property -- hhlee
            wordlist = sorted (csvfile, key=operator.itemgetter (0,1,2,3))
	    ...
	    row_num = 0 # to record the sequence of the list
            for k, g in itertools.groupby (wordlist, key=operator.itemgetter (0,1,2,3)):
                groups.append ([row_num, list (g), sum(float(v[4]) for v in g)])
       		row_num = row_num + 1         
		uniquekeys.append (k)

	    # below is to calculate the sum of the values of a group(house_id, household_id, plug_id, property)
	    group_sum = []
            row_num = 0
            for k, g in itertools.groupby (wordlist, key=operator.itemgetter(0,1,2,3)):
                group_sum.append([row_num, sum(float(v[4]) for v in g)])
                row_num = row_num +1	    

	    # below is to calculate the max timestamp value of a group
	    max_timestamp_list = []
            row_num = 0
            for k, g in itertools.groupby(wordlist, key=operator.itemgetter(0,1,2,3)):
                max_timestamp_list.append([row_num, max(k[5] for k in g)])
                row_num = row_num+1

	    # below is to calculate the min timestamp value of a group
            min_timestamp_list = []
	    row_num = 0
            for k, g in itertools.groupby(wordlist, key=operator.itemgetter(0,1,2,3)):
                min_timestamp_list.append([row_num, min(k[5] for k in g)])
                row_num = row_num+1


	    # below is to calculate the avg load value and avg work value
	    avg_load = 0.0
            avg_work = 0.0
            sum_value = 0.0
            group_count = 0
	    ...
	    for j in range (len(uniquekeys)):
            	sum_value = group_sum[j][1] # value sum
                group_count = len(groups[j][1]) # group count
		
		# tempfile write attributes : house_id, household_id, plug_id, property, sum of values, max_timestamp, min_timestamp, group_count
                tempfile.write(uniquekeys[j][0]+str(", ")+uniquekeys[j][1]+str(", ")+uniquekeys[j][2]+str(", ")+uniquekeys[j][3]+str(", "))
                tempfile.write(str(sum_value)+", "+max_timestamp_list[j][1]+", "+min_timestamp_list[j][1]+", "+str(group_count)+"\n")

                # if int(uniquekeys[j][3]) == 0:
		#	if group_count == 0:
		#		avg_load = 0
		#	else:	
                #        	avg_load = sum_value / group_count 
	
                #       	tempfile.write(str(avg_load)+"\n")
                #else:
		#	if (float(max_timestamp_list[j][1]) - float(min_timestamp_list[j][1])) == 0.0:
		#		avg_work = 0.0
		#	else:
                #        	avg_work = sum_value / (float(max_timestamp_list[j][1]) - float(min_timestamp_list[j][1]))

	        #       tempfile.write(str(avg_work)+"\n")
        ...
        ##### rows = [[int(row[0]), int(row[1]), int(row[2]), int(row[3]), float(row[4]), int(row[5]), int(row[6])] for row in csvfile]
	...
	intmed_key_val_list = []
		...
		intmed_key_val_list.append({'house_id': wordlist[i][0], 'household_id': wordlist[i][1], 'plug_id': wordlist[i][2], 'property': wordlist[i    ][3], 'sum': wordlist[i][4], 'max_timestamp': wordlist[i][5], 'min_timestamp': wordlist[i][6], 'group_count': wordlist[i][7]})

	
            pickle.dump (intmed_key_val_list, shufflefile, pickle.HIGHEST_PROTOCOL)
