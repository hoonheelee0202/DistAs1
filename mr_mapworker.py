...
class MR_Map ():
	...
	# tokenize the incoming chunk, which is a string. We want the
        ...
        intmed_key_val_list = []
        ### read line by line -- hhlee
        ####### contents_read = content.readlines()

        ### pattern is comma(,)
        pattern = "(\,)"

	print ("Right before for")
	for i in content.splitlines():
		split_arg = re.split(pattern, i)
	
        # tokenize the stream according to the pattern above
        ####### for i in contents_read:
	##### for i in content:
            ##### i = i.strip()
            ##### split_arg = re.split(pattern, i)
		#print split_arg
	    	intmed_key_val_list.append({'house_id': split_arg[12], 'household_id': split_arg[10], 'plug_id': split_arg[8], 'property': split_arg[6], 'value': split_arg[4], 'timestamp': split_arg[2], 'id': split_arg[0]})

	    	#intmed_file.write (split_arg[12]+", "+split_arg[10]+", "+split_arg[8]+", "+split_arg[6]+", "+split_arg[4]+", "+split_arg[2]+", "+split_arg[0]+"\n")
	print ("Right after for")
	#intmed_file.close()
