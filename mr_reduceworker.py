...
class MR_Reduce ():
	...
	for p in range(len(content)):
		sum_value = float(content[p]['sum']) # value sum
                group_count = int(content[p]['group_count']) # group count
		time_diff = float(content[p]['max_timestamp']) - float(content[p]['min_timestamp'])

                if int(content[p]['property']) == 0:
                       if group_count == 0:
                               avg_load = 0
                       else:
                                avg_load = sum_value / group_count
				
		       key_val_list.append({'house_id': content[p]['house_id'], 'household_id': content[p]['household_id'], 'plug_id': content[p]['plug_id'], 'property': content[p]['property'], 'sum': str(sum_value), 'time': str(time_diff), 'group_count': str(group_count), 'avg_value': str(avg_load)})
                else:
                        if (time_diff) == 0.0:
                               avg_work = 0.0
                        else:
                               avg_work = sum_value / time_diff
		
			key_val_list.append({'house_id': content[p]['house_id'], 'household_id': content[p]['household_id'], 'plug_id': content[p]['plug_id'], 'property': content[p]['property'], 'sum': str(sum_value), 'time': str(time_diff), 'group_count': str(group_count), 'avg_value': str(avg_work)})



	#	print("house_id: "+p['house_id']+", household_id: "+p['household_id'])
	#for i in content.splitlines():
	#	j_content = json.load(i)
	#	print j_content
	    ...
	    #key_val_list.append({'house_id': items[i][0], 'household_id': wordlist[i][1], 'plug_id': wordlist[i][2], 'property': wordlist[i][3], 'sum': wordlist[i][4], 'max_timestamp': wordlist[i][5] 'min_timestamp': wordlist[i][6], 'group_count': wordlist[i][7]})
	
	#key_val_list = content
